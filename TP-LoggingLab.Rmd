...
We will use the LoggingLab package for R [@Badouard2024] to simulate reduced-impact logging in a tropical forest.
...
- `idTree`: a unique identifier for each tree. 
...
TimberV
```
Details of the yield by species is in the inventory.

```{r}
TimberV$inventory %>% 
  # Cut trees
  filter(DeathCause == "cut") %>% 
  # Group by commercial name
  group_by(CommercialName) %>% 
  # Number of trees, timber volume and commercial level
  summarise(
    trees_n = n(), 
